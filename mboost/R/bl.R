...
df2lambda <- function(X, df = 4, lambda = NULL, dmat = NULL, weights,
    ...
    if (!is.null(df)) {
        ...
        if (df >= rank_X) {
            ...
            return(c(df = df, lambda = 0, dfTrS = df))
        }
    }
    if (!is.null(lambda))
        if (lambda == 0)
            return(c(df = rankMatrix(X), lambda = 0, dfTrS = rankMatrix(X)))
    ...
    if (!is.null(lambda)){
        dfTrS <- sum(1 / (1 + lambda * d))
        return(c(df = dfFun(lambda), lambda = lambda, dfTrS = dfTrS))
    }
    ...
    return(c(df = df, lambda = lambda, dfTrS = dfTrS))
}
...
X_ols <- function(mf, vary, args) {
    ...
    if (isMATRIX(mf)) {
        ...
        X <- model.matrix(as.formula(fm), model.frame(as.formula(fm), mf, na.action = na.pass), contrasts.arg = args$contrasts.arg)
        if (DUMMY)
...
bl_lin <- function(blg, Xfun, args) {
    ...
    newX <- function(newdata = NULL, prediction = FALSE) {
        ...
        ## prediction argument is currently only used in X_bbs --> bsplines
        args$prediction <- prediction
        Xtemp <- Xfun(mf, vary, args)
        return(Xtemp)
    }
    X <- newX()
    K <- X$K 
    ...
    dpp <- function(weights) {
        ...
        fit <- function(y) {
            if (!is.null(index)) {
                if( any(is.na(index)) ) na_offset <- mean(y[is.na(index)])
                else na_offset <- 0
                if (is.matrix(y)) {
            ...
            ret <- list(model = coef,
                        ...
                        fitted = function() {
                            ...
                            ret2 <- ret[index] 
                            ret2[is.na(ret2)] <- na_offset #average of y values for NA                          
                            return(ret2)
                        }
                        )
        ...
        predict <- function(bm, newdata = NULL, aggregate = c("sum", "cumsum", "none")) {
            cf <- sapply(bm, coef) 
            na_off <- sapply(bm, function(b) b$na_offset)
            if (!is.matrix(cf))
                cf <- matrix(cf, nrow = 1)
            if(!is.null(newdata)) {
                index <- NULL
            ...
            pr <- switch(aggregate, "sum" = {
                ...
                if(!is.null(newdata)){
                    ret[is.na(ret)] <- sum(na_off)
                } else {
                    #similar treatment to fit()
                    #X, index are set at top of bl_lin
                    if (!is.null(index)){
                        tmp <- ret[index]
                        tmp[is.na(tmp)] <- sum(na_off)                       
                        ret <- tmp
                    }    
                }
                matrix(ret, ncol = P)
            ...
            "cumsum" = {
                stopifnot(P == 1L)
                ret <- as(X %*% .Call("R_mcumsum", as(cf, "matrix"),
                            PACKAGE = "mboost"), "matrix")
                if(!is.null(newdata)){
                    if(any(!complete.cases(ret))){
                        off_mat <- matrix(na_off, nrow = sum(!complete.cases(ret)), 
                                          ncol = length(na_off), byrow = T)
                        off_cumsum <- as(.Call("R_mcumsum", as(off_mat, "matrix"),
                                         PACKAGE = "mboost"), "matrix")
                        ret[!complete.cases(ret),] <- off_cumsum
                    }
                } else {
                    #similar treatment to fit()
                    #X, index are set at top of bl_lin
                    if (!is.null(index)){
                        tmp <- ret[index,]
                        off_mat <- matrix(na_off, nrow = sum(!complete.cases(tmp)), 
                                          ncol = length(na_off), byrow = T)
                        off_cumsum <- as(.Call("R_mcumsum", as(off_mat, "matrix"),
                                         PACKAGE = "mboost"), "matrix")
                        tmp[!complete.cases(tmp),] <- off_cumsum                     
                        ret <- tmp
                    }  
                }
            },
            "none" = {
                ...
                if(!is.null(newdata)){
                    if(any(!complete.cases(ret))){
                        ret[!complete.cases(ret),] <- matrix(na_off, nrow = sum(!complete.cases(ret)), ncol = length(na_off), byrow = T)
                    }
                } else {
                    #similar treatment to fit()
                    #X, index are set at top of bl_lin
                    if (!is.null(index)){
                        tmp <- ret[index,]
                        tmp[!complete.cases(tmp),] <- matrix(na_off, nrow = sum(!complete.cases(tmp)), ncol = length(na_off), byrow = T)                       
                        ret <- tmp
                    }  
                }
                ret
